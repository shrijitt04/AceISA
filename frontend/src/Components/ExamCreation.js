...
const ExamCreationPage = () => {
  ...
  const handleNextQuestion = () => {
    if (questionText && options.every(option => option) && correctAnswer !== null) {
      const newQuestion = {
        text: questionText,
        options,
        correctAnswer,
      ...
      alert('Please fill in all fields and select a correct answer before proceeding.');
    }
  };
  ...
  return (
    ...
    >
      <h1 className={`text-center mb-5 text-white ${showTitle ? 'fade-in' : ''}`} style={{ fontSize: '3.5rem' }}>
        Exam Creation
      </h1>
      ...
      {showQuestionForm && (
        <div className={`question-form ${showQuestionForm ? 'fade-in' : ''}`}>
          ...
          <div className="mb-3">
            ...
            />
            {image && <img src={image} alt="Question" className="img-fluid mt-2" style={{ maxHeight: '200px' }} />}
          </div>
